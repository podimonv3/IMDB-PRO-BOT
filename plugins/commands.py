...
# ====================================================================
# === വിക്കിപീഡിയ സിനിമകൾ ശേഖരിക്കാനുള്ള കസ്റ്റം കമാൻഡ് (FINAL WORKING) ===
# ====================================================================
...
async def fetch_wiki_movies_command(client, message):
    ...
    user_id = message.from_user.id
    USER_MOVIE_DATA[user_id] = {
        "movies": movie_list,
        "current_index": 0
    }
    
    total = len(movie_list)
    first_movie = movie_list[0] # ആദ്യ സിനിമ കൃത്യമായി എടുക്കുന്നു
...
async def navigate_wiki_movies_fixed(client, callback_query):
    ...
    action = data_split[1] # ഇവിടെ [1] ചേർത്തതോടെ എറർ പൂർണ്ണമായി മാറി
    ...
    current_movie = movies[next_index]
    
    reply_text = f"🎬 **സിനിമ {next_index + 1}/{total}**\n\nകോപ്പി ചെയ്യാൻ താഴെ കാണുന്ന പേരിൽ തൊടുക:\n\n`{current_movie}`"
